...
- github copy at `~/cortexagent-github/` uses base **IQ3_S** (no UD).
- **No fallback model** — both copies ship `fallback_model = ""`. User: "I just want the UD model."
- `idle_unload_sec = 0` — big stays loaded.

### Branding
- "CortexAgent by the maintainer" in `bin/cortexagent:2`, `README.md:7`, and `lib/config.py:355` (author default).
...
| # | When | Item | Files | Why |
|---|------|------|-------|-----|
| 38 | Aug 17 | **Cortex processing-core animation** — new `lib/processing_animation.py` (per `docs/ANIMATION-SPEC.md`): fixed 7-row 'CORTEX / ACTIVE' card above the input while a request is in flight, one animation per high-level stage (context-prep orbiting nodes → SlimToken compress → sending packets → generating waveform → tool wait → completion ✓ → error !). Real progress % only (indeterminate sweep for generation/unknown, never a fake %); 4-frame cycles at 180 ms; reduced-motion → static frame 0 (`CORTEXAGENT_REDUCED_MOTION=1`); narrow terminals auto-shrink (≤54 cols one-line, <40 cols `◈ Working · %`). `ProcessingAnimation` in-place renderer (cursor-up + erase, never appends frames). `stage_from_workphase()` maps the TUI's `WorkPhase` → stage. Wired into `lib/tui.py` `_status_work` (replaces the single-line work indicator). Smoke test `test_processing_animation` registered in `tests/run_smoke.py` (tui area) — 6/6 green. | `lib/processing_animation.py` (new), `lib/tui.py`, `tests/run_smoke.py` | User: 'when they're done. You can do the animations... it will show like percentage of things' |
| 37 | Aug 17 | **STT fixed + locked: faster-whisper base on GPU, VAD 0.03, big 96k/ub2560** — root cause of STT garbage/singing = the 6 GB CUDA gate (`_STT_CUDA_MIN_FREE_MIB`) forced whisper to CPU int8 whenever the big model was loaded; CPU whisper hallucinated (talk-show prompts, 'you/why/oh', singing). Fix: `stt_device auto→cuda` (bypasses gate), `stt_model tiny→base` (faster-whisper, ~2x more accurate, 394 MiB on GPU), `stt_vad_threshold 0.05→0.03` (0.05 was too insensitive — mic RMS 0.0035, voice never triggered). Big model retuned 96k ctx / ub 2560 (14,216 MiB) + `CORTEXAGENT_MINIFY_BUDGET=98304` in systemd unit (was 131072 > 96k window → 400-error). Backup commit `7986002` on branch `backup/stt-config-2026-08-17` (config.py + conf + systemd unit). STT daemon supervised by tray (respawns via `voice start`); log at `~/.cortexagent/stt_daemon.log`. | `lib/config.py`, `lib/stt.py`, `~/.cortexagent/cortexagent.conf`, `~/.config/systemd/user/cortexagent.service` | User: 'it's really, really bad at it' + 'make sure it stays locked in' |
| 36 | Aug 13 | **Full harness: tool support for ANY model + MCP client + browser + skills + beautify** — text-based tool-call fallback (`_parse_text_tool_calls`: JSON shapes + ReAct + `<function_call>` tags) so any text-JSON-emitting model drives the react loop; `lib/mcp_client.py` (persistent stdio MCP client, registers `mcp_<server>_<tool>`, opt-in via `CORTEXAGENT_MCP_SERVERS`); `lib/browser_tools.py` (9 `brave_*` tools direct, no MCP round-trip); `lib/skills.py` (skill modules from `~/.cortexagent/skills/` → `skill_<name>`); `lib/beautify.py` (tables + ASCII bar charts, prose-safe); `lib/harness_tools.py` wiring (idempotent `ensure_registered()`); tool registry priority ordering + `list_tools(limit)`; `MAX_TOOLS=16` keeps the surface inside the tiny model's 2048 ctx | `lib/tiny_llm.py`, `lib/react_loop.py`, `lib/mcp_client.py`, `lib/browser_tools.py`, `lib/skills.py`, `lib/beautify.py`, `lib/harness_tools.py`, `lib/tool_registry.py`, `tests/run_smoke.py` | User: "full tool support for ANY model" + "full harness on par with opencode/openclaw, everything into skills and mcp servers" |
...
| 33 | Aug 12 | **STT English-only** — `transcribe()` passes `language="en"` (user: "disable all other languages but English"). Forces English output AND skips the language-detection pass (faster). STT smoke 7/7 green. Needs STT daemon restart to take effect. | `lib/stt.py` | User: "disable all other languages but English" |
| 34 | Aug 13 | **Overseer abliterated-model experiment + REVERT** — tested `DeepSeek-R1-Distill-Qwen-1.5B-Abliterated-dpo.i1-Q4_K_M` (user's pick, 1.12 GB) and `Qwen2.5-1.5B-Instruct-abliterated.Q4_K_M` (941 MB) as overseer candidates. **Both REJECTED**: mradermacher abliteration broke their embedded chat templates (degenerate output — raw completion fine, chat garbage) AND native tool calls (respond with text, never `tool_calls`; text-based single JSON works but multi-step ReAct fails — DeepSeek repeats first call, Qwen2.5 describes). Forcing `--chat-template chatml` fixes plain chat but not tool calls. Reverted overseer to LFM2.5-1.2B (native tool calls + ReAct loop verified working). Config + divergence doc reverted. Models left on disk (1.12 GB + 941 MB) pending user decision. | `~/.cortexagent/cortexagent.conf`, `docs/CORTEXLLM-0.4.0-DIVERGENCE.md` | User: "we'll use a better model with tool support like the other one being downloaded" + "ok just stick to what we have for now" |
...
| 16 | Aug 11 | **Branding: 'CortexAgent by the maintainer'** — consistent across launcher, README, config author default. Added `README.md` + `bin/cortexagent` to PII exclude (authorship is not PII). | `bin/cortexagent:2`, `README.md:7`, `lib/config.py:355`, `tests/run_smoke.py` PII_EXCLUDE_FILES | User feedback: "it should say CortexAgent by the maintainer but it says CortexAgent by CortexAgent". Authorship ≠ PII. |
...
| 13 | Aug 11 | **GitHub copy prepared** at `~/cortexagent-github/` — 217 files / 6.8 MB, PII scrubbed (install paths, install handles, API key prefixes, model quant tags). Old changelogs removed (Aug 4 overseer-registry design, output-rules-design). `big_model` default = base IQ3_S (no UD), `fallback_model = ""`, version 0.3.2. Git initialized + commit `5c20b87`. | `~/cortexagent-github/` (full copy) | User: "purge any PII from the public repo... remove any old changelogs that have personal info". |
...
| 7 | Aug 10 | **Slimtoken as primary minify backend** in `lib/grammar_proxy.py` — real compression (dedup + distill + 131072 backstop budget). lib.minify kept as fallback. Auto-detected; env-overridable. | `lib/grammar_proxy.py` (+47) | Better compression than stdlib lib.minify; already shipped at v0.3.3 on `<repo>/slimtoken`. |
...
Source: `<tray-icon-source>` (768×1376 RGB)
