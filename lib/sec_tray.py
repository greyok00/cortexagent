...
import lib.sec_controls as _sec
...
def _update_icon_now(icon) -> None:
    ...
    try:
        alerts = _sec._merge_and_sort(limit=_ALERT_LIMIT)
    except Exception:
...
def main() -> int:
    ...
    try:
        ...
        print(f"[sec-tray] tray exit error (swallowed): {exc}",
              file=sys.stderr)
    finally:
        stop_event.set()
