...
TOOLS = [
    _tool("chrome_status", "Check Chrome CDP reachability and count open tabs.", {}, []),
    _tool("chrome_tabs", "List open tabs (index, title, url).", {}, []),
    _tool("chrome_navigate", "Navigate a tab to URL; return title+URL.", {
        "url": {"type": "string"},
        "tab": _TAB,
    }, ["url"]),
    _tool("chrome_fetch", "Fetch page text via Chrome (use for JS-heavy sites).", {
        "url": {"type": "string"},
    ...
    _tool("chrome_click", "Click element by CSS selector or accessible text.", {
        "target": {"type": "string"},
        "by_text": {"type": "boolean"},
        "timeout": {"type": "number", "description": "Seconds (default 10)."},
        "tab": _TAB,
    }, ["target"]),
    _tool("chrome_type", "Type text into an element.", {
        "target": {"type": "string"},
        "text": {"type": "string"},
        "by_text": {"type": "boolean"},
        "submit": {"type": "boolean"},
    ...
    _tool("chrome_evaluate", "Evaluate JS in Chrome and return JSON result.", {
        "expression": {"type": "string"},
        "timeout": {"type": "number", "description": "Seconds (default 10)."},
        "tab": _TAB,
    }, ["expression"]),
    _tool("chrome_snapshot", "Return accessibility snapshot of a tab.", {
        "depth": {"type": "number"},
        "tab": _TAB,
    }, []),
    _tool("chrome_fill_send", "Fill a shadow-DOM controlled component (React/LWC) and press Enter. Use for embedded chat composers.", {
        "text": {"type": "string"},
...
def _handle_request(req: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if method == "initialize":
        try:
            ...
            pass
        return {"jsonrpc": "2.0", "id": _id, "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "patchright-chrome-mcp", "version": "1.3"}}}
